...
export async function loadPems(issuer: string): Promise<Pems> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed fetching jwks.json: ${response.status} ${response.statusText}\n\n${body}`);
  }
...
export const validate = async (config: CognitoConfig, token: string): Promise<CognitoValidationResult> => {
